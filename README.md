...
```bash
get-posts$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
get-posts$ sam local start-api
get-posts$ curl http://localhost:3000/
```
...
```bash
get-posts$ sam logs -n HelloWorldFunction --stack-name get-posts --tail
```
...
```bash
aws cloudformation delete-stack --stack-name get-posts
```
